        ...
        if (index != -1){
            ...
            submitButton.setOnClickListener {
                if(idEditText.text.toString() == "" || nameEditText.text.toString() == ""){
                    idEditLayout.error = "ID tidak boleh kosong!!"
                    nameEditLayout.error = "Nama tidak boleh kosong!!"
                    Toast.makeText(context, "Nama dan ID tidak boleh kosong!!", Toast.LENGTH_LONG).show()
                } else {
                    val student = Student(
                        id = index,
            ...
            submitButton.setOnClickListener {
                if(idEditText.text.toString() == "" || nameEditText.text.toString() == ""){
                    ...
                    val action: NavDirections = StudentFormFragmentDirections.actionStudentFormFragmentToStudentListFragment()
                    view.findNavController().navigate(action)
                    Toast.makeText(context, "${student.name} berhasil ditambahkan!!", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
